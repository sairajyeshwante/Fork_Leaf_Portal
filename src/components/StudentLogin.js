...
import base_url from "../API/ServiceAPI"; // Ensure this points to your backend base URL
...
const StudentLogin = () => {
  ...
  const onLogin = () => {
    ...
    // Prepare the payload. Adjust the payload if the backend expects the course as well.
    const payload = { id: prn, password, courseName: course};

    axios
      .post(`${base_url}/studLogin`, payload)
      .then((response) => {
        console.log("Login Successful:", response.data);
        navigate("/studentHome"); // Redirect to student home on success
      })
      .catch((error) => {
